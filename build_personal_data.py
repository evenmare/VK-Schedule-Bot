...
class buildPersonalData:
    ...
    def selectUserInfo(self, perID):
        ...
        operationRequest = "SELECT perID, name, eveningNotification, morningNotification, 30minNotification, 5minNotification, localLessons, distantLessons FROM mainTable WHERE perID = %s"
    ...
    def insertNewUserInfo(self, perID, name):
        ...
        operationRequest = "INSERT INTO mainTable (perID, name, eveningNotification, morningNotification, 30minNotification, 5minNotification, localLessons, distantLessons) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"
    ...
    def updateUserInfo(self, perID, eveningNotification, morningNotification, min30Notification, min5Notification, localLessons, distantLessons):
        operationCursor = self.dbConnection.cursor()
        operationRequest = "UPDATE mainTable SET eveningNotification = %s, morningNotification = %s, 30minNotification = %s, 5minNotification = %s, localLessons = %s, distantLessons = %s WHERE perID = %s"
    ...
    def deleteUserInfo(self, perID):
        operationCursor = self.dbConnection.cursor()
        operationRequest = "DELETE FROM mainTable WHERE perID = %s"
        operationValues = (perID, )
        operationCursor.execute(operationRequest, operationValues)
        self.dbConnection.commit()
    ...
    def selectAllUsers(self):
        operationCursor = self.dbConnection.cursor()
        operationRequest = "SELECT perID FROM mainTable"
    ...
    def selectTimeNotifications(self, time_type_of_notification):
        operationCursor = self.dbConnection.cursor()
        operationRequest = "SELECT perID FROM mainTable WHERE " + time_type_of_notification + "Notification = 1"
        operationCursor.execute(operationRequest)
        operationResult = operationCursor.fetchall()
        return self.formattingSelectQuery(operationResult)
    ...
    def selectLocationNotifications(self, location_type_of_notification):
        operationCursor = self.dbConnection.cursor()
        operationRequest = "SELECT perID FROM mainTable WHERE " + location_type_of_notification + "Lessons = 1"
        operationCursor.execute(operationRequest)
        operationResult = operationCursor.fetchall()
        return self.formattingSelectQuery(operationResult)
